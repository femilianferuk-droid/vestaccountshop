...
app = Flask(__name__)
app.secret_key = 'vest_account_clean_minimal_fixed'
...
CRYPTO_BOT_URL = 'https://pay.crypt.bot/api/'  # Обновленный URL по вашему запросу
...
# Синхронизация структуры таблиц и цен
...
# --- Шаблон интерфейса (Премиальный Светлый Минимализм) ---
BASE_TEMPLATE = """
<!DOCTYPE html>
<html lang="ru">
<head>
...
def buy_account(country):
    ...
    # ИСПРАВЛЕНО: Принудительный сброс кэша сессии СУБД, чтобы получить свежий баланс без задержек
    db.session.expire_all()
    user = db.session.get(User, session['user_id'])
    
    clean_country = country.strip()
    
    # ИСПРАВЛЕНО: Безопасное сопоставление строки на стороне Python для обхода багов локали PostgreSQL
    matched_country = next((c for c in COUNTRIES_LIST if c.lower() == clean_country.lower()), None)
    if not matched_country:
        flash('Выбранная страна не поддерживается платформой.', 'error')
        return redirect(url_for('shop'))
        
    price_rec = CountryPrice.query.get(matched_country)
    ...
    # ИСПРАВЛЕНО: Строгий поиск по точному совпадению строки из проверенного пула
    account = Account.query.filter_by(country=matched_country, is_sold=False).first()
    if not account:
...
def purchases():
    ...
    {% for p in my_purchases %}
        <div class="card" style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:12px; padding:14px 20px;">
            <div style="font-weight:600; font-size:14px;">{{ flags.get(p.country, '') }} {{ p.country }}: <code style="color:#0066cc; font-size:15px; background:#f6f8fa; padding:3px 6px; border-radius:4px; border:1px solid #d0d7de;">{{ p.phone }}</code></div>
            <div>
...
def admin_verify_account():
    ...
    # ИСПРАВЛЕНО: Запись строго сопоставленной строки страны из константного пула
