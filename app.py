...
# ИСПРАВЛЕНО: Расширен список гео-позиций новыми странами
COUNTRY_MAP = {
    'ru': {'name': 'Россия', 'flag': '🇷🇺'},
    'us': {'name': 'США', 'flag': '🇺🇸'},
...
<head>
    ...
    <style>
        ...
        /* ИСПРАВЛЕНО: Бесконечный синий неоновый перелив для названия сайта */
        ...
        /* ИСПРАВЛЕНО: Постоянный перелив для ВСЕХ кнопок на платформе */
...
# ИСПРАВЛЕНО: Новый макет профиля (Никнейм + Баланс + Переливающаяся кнопка перехода)
@app.route('/profile')
def profile():
    if not session.get('user_id'): return redirect(url_for('login'))
    db.session.expire_all()
    ...
    <div class="glass p-6 rounded-2xl max-w-md mx-auto">
        ...
        <a href="/deposit_page" class="w-full block text-center py-3.5 text-sm font-bold text-white btn-shimmer rounded-xl shadow-lg transition-transform active:scale-[0.98]">
            Пополнить баланс
        </a>
...
# ИСПРАВЛЕНО: Страница пополнения баланса (Выбор Crypto Bot + Ввод суммы в рублях)
...
def deposit_page():
    ...
    <div class="glass p-6 rounded-2xl max-w-md mx-auto border border-white/5">
        ...
        <form action="/deposit" method="POST" class="space-y-4">
            ...
            <div>
                <label class="block text-xs font-bold uppercase text-slate-400 mb-2">Сумма пополнения (в Рублях, ₽)</label>
...
def deposit():
    ...
    amount_usdt = amount_rub / 90.0  # Скрытый курс: 1 USDT = 90 рублей
    ...
    try:
        ...
        if r.get('ok'):
            data = r['result']
            inv = Invoice(user_id=session['user_id'], crypto_invoice_id=data['invoice_id'], amount=float(amount_usdt))
            db.session.add(inv)
            db.session.commit()
            html = """
            <div class="glass p-8 rounded-2xl text-center max-w-md mx-auto">
                ...
                <p class="text-slate-400 text-sm mb-6">Для пополнения на сумму <b>{{ amount_rub }} ₽</b> перейдите к оплате:</p>
                ...
                <form action="/check_invoice/{{ inv_id }}" method="POST">
            ...
            """
            return render_page(html, active_tab='profile', url=data['pay_url'], amount_rub=int(amount_rub), inv_id=inv.id)
        ...
        <div class="glass p-6 rounded-xl">
            ...
            <form method="POST" class="space-y-4"><input type="hidden" name="change_balance" value="1">
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <input type="number" step="0.01" name="balance" placeholder="Сумма в ₽" required class="w-full p-3 bg-slate-955 border border-white/10 rounded-xl text-white focus:outline-none">
                </div>
