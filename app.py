...
app = Flask(__name__)
app.secret_key = 'vest_account_dark_final_premium_2026'
...
# Синхронизация структуры и миграции таблиц на лету
...
# --- ШАБЛОН ОДИН В ОДИН КАК НА ТВОЕМ СКРИНШОТЕ ---
BASE_TEMPLATE = """
<!DOCTYPE html>
<html lang="ru">
<head>
    ...
    <style>
        body { 
            ...
            background: radial-gradient(circle at top, #111a2e 0%, #070913 100%) no-repeat fixed; 
            color: #ffffff; margin: 0; padding-bottom: 95px;
            -webkit-font-smoothing: antialiased;
        }
        .header { 
            display: flex; justify-content: space-between; align-items: center; 
            padding: 14px 20px; background: rgba(13, 17, 30, 0.4); border-bottom: 1px solid rgba(255,255,255,0.02);
            backdrop-filter: blur(15px); -webkit-backdrop-filter: blur(15px);
        }
        
        .logo-block { display: flex; align-items: center; gap: 10px; text-decoration: none;}
        .logo-square { 
            width: 36px; height: 36px; background: #0088ff; 
            border-radius: 8px; display: flex; align-items: center; justify-content: center;
            font-weight: 900; font-size: 18px; color: #ffffff; box-shadow: 0 0 12px rgba(0,136,255,0.4);
        }
        .logo-text { display: flex; flex-direction: column; }
        .logo-title { font-size: 16px; font-weight: 700; color: #ffffff; letter-spacing: -0.3px; }
        .logo-subtitle { font-size: 10px; color: #536485; font-weight: 700; letter-spacing: 1px; }

        /* ИСПРАВЛЕНО: Кнопки «Войти» / «Регистрация» по дизайну скрина (ярко-синие, видимые) */
        .auth-buttons-visible { display: flex; gap: 8px; }
        .auth-buttons-visible a {
            background: #4cc3ff; color: #070913; text-decoration: none; padding: 8px 14px;
            border-radius: 10px; font-size: 13px; font-weight: 700; transition: background 0.2s;
        }
        .auth-buttons-visible a:hover { background: #0088ff; color: #ffffff; }
        ...
        .balance-label { font-size: 9px; color: #536485; font-weight: 800; letter-spacing: 0.5px; }
        ...
        .container { max-width: 540px; width: 92%; margin: 24px auto; }
        .section-label { font-size: 11px; font-weight: 700; color: #536485; letter-spacing: 1px; text-transform: uppercase; margin-bottom: 14px; }
        
        /* Карточки товаров */
        .card { 
            background: rgba(20, 26, 46, 0.5); border: 1px solid rgba(255,255,255,0.03); border-radius: 16px; 
            padding: 20px; margin-bottom: 16px; box-shadow: 0 4px 20px rgba(0,0,0,0.2);
        }
        
        .category-card {
            background: rgba(20, 26, 46, 0.4); border: 1px solid rgba(255,255,255,0.02); border-radius: 16px; 
            padding: 16px; margin-bottom: 12px; display: flex; justify-content: space-between; align-items: center;
            text-decoration: none; transition: all 0.2s ease;
        }
        .category-card:hover { background: rgba(30, 39, 69, 0.5); border-color: rgba(0,136,255,0.2); }
        .category-left { display: flex; align-items: center; gap: 14px; }
        .category-icon-wrapper {
            width: 42px; height: 42px; background: rgba(255,255,255,0.02); border: 1px solid rgba(255,255,255,0.02);
        ...
        .category-desc-text { font-size: 11px; color: #536485; margin-top: 2px; }
        .category-right { display: flex; align-items: center; gap: 10px; }
        .category-price-tag { font-size: 14px; font-weight: 700; color: #4cc3ff; }
        .category-arrow { color: rgba(255,255,255,0.1); font-size: 16px; }

        /* ИСПРАВЛЕНО: Кнопка «Купить» строго справа, переливается */
        @keyframes blueFlow {
        ...
        .btn-buy-fixed {
            ...
            animation: blueFlow 3s linear infinite; color: #ffffff; border: none;
        ...
        .btn-buy-fixed:active { transform: scale(0.96); }

        .btn-action { 
            background: linear-gradient(90deg, #0052cc, #0088ff, #0052cc); background-size: 200% auto;
            color: #ffffff; border: none; padding: 12px 20px; border-radius: 12px; 
            cursor: pointer; font-weight: 700; text-decoration: none; display: inline-block; text-align: center; font-size: 14px;
            animation: blueFlow 3s linear infinite; width: 100%; box-sizing: border-box;
        }
        .btn-secondary { background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.05); color: #ffffff; animation: none; }

        .country-inline-row { display: flex; flex-direction: column; gap: 8px; }
        .country-line-item {
            display: flex; justify-content: space-between; align-items: center;
            background: rgba(20, 26, 46, 0.3); border: 1px solid rgba(255,255,255,0.01); border-radius: 14px; padding: 12px 16px;
        }
        .country-info { display: flex; align-items: center; gap: 12px; font-weight: 600; font-size: 14px; }
        .country-flag { font-size: 20px; }
        
        /* Схематичный нижний бар со скриншота */
        .bottom-nav { 
            position: fixed; bottom: 0; left: 0; right: 0; height: 65px; 
            background: rgba(7, 9, 19, 0.9); border-top: 1px solid rgba(255,255,255,0.02); 
            display: flex; justify-content: space-around; align-items: center; z-index: 999;
            backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px);
        }
        .nav-item { 
            display: flex; flex-direction: column; align-items: center; gap: 4px;
            text-decoration: none; color: #4e566d; padding: 6px 20px; border-radius: 12px; transition: all 0.2s;
        }
        .nav-text-box { font-size: 11px; font-weight: 700; letter-spacing: 0.2px; }
        .nav-item.active { 
            color: #0088ff; background: radial-gradient(circle, rgba(0,136,255,0.1) 0%, rgba(0,136,255,0) 70%);
        }
        
        .form-group { margin-bottom: 14px; }
        .form-group label { display: block; margin-bottom: 6px; color: #536485; font-size: 12px; font-weight: 700; text-transform: uppercase; }
        .form-group input, .form-group select { 
            width: 100%; padding: 12px; box-sizing: border-box; background: rgba(0,0,0,0.2); 
            border: 1px solid rgba(255,255,255,0.04); color: #ffffff; border-radius: 10px; font-size: 14px;
        }
        .form-group input:focus { border-color: #0088ff; outline: none; }
        ...
        th, td { padding: 12px 10px; text-align: left; border-bottom: 1px solid rgba(255,255,255,0.02); white-space: nowrap; }
        th { background: rgba(255,255,255,0.01); color: #536485; font-weight: 600; }
    </style>
</head>
<body>
    <div class="header">
        ...
        <div>
            {% if not session.get('user_id') %}
                <div class="auth-buttons-visible">
...
def index():
    ...
    <p style="color:#536485; font-size:13px; margin: 0 0 24px 0;">Telegram-аккаунты — в один тап</p>
...
def shop_category(cat_slug):
    ...
    <p style="color:#536485; font-size:13px; margin-bottom:20px;">Кнопка «Купить» расположена строго справа</p>
        ...
        {% for c_slug, data in country_map.items() %}
            <div class="country-line-item">
                ...
                <div style="display:flex; align-items:center; gap:16px;">
                    <span style="font-size:14px; font-weight:700; color:#ffffff;">{{ prices.get(data.name, 300) }} ₽</span>
                    <form action="/buy/{{ cat_slug }}/{{ c_slug }}" method="POST" style="margin:0;">
                        <button type="submit" class="btn-buy-fixed">Купить</button>
                    </form>
                </div>
            </div>
        {% endfor %}
...
def purchases():
    ...
    {% if not my_purchases %}<p style="color:#536485; font-size:14px;">У вас нет совершенных покупок.</p>{% endif %}
    {% for p in my_purchases %}
        <div class="card" style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:12px; padding:16px;">
            <div style="font-size:14px; font-weight:600;">
                <span style="color:#536485; font-size:11px;">{{ cat_names.get(p.category) }}</span><br>
                {{ p.country }}: <code style="color:#4cc3ff; font-size:15px; background:rgba(0,0,0,0.2); padding:2px 6px; border-radius:6px; border:1px solid rgba(255,255,255,0.03);">{{ p.phone }}</code>
            </div>
            <div>
                <button class="btn-buy-fixed" style="animation:none;" onclick="getCode({{ p.account_id }}, this)">Получить код</button>
...
def profile():
    ...
    <div class="card">
        ...
        <p style="color:#536485; font-size:13px; margin-bottom:2px;">Текущий баланс:</p>
    ...
    try:
        ...
        if r.get('ok'):
            ...
            <div class="card" style="text-align:center; padding:24px;">
                <h3>Счет готов</h3>
                <p style="color:#536485; font-size:14px; margin-bottom:20px;">Для оплаты нажмите кнопку ниже:</p>
...
def admin_add_account():
    ...
    <div class="card" style="max-width:400px; margin:auto;">
        <h3>Ввод SMS-кода</h3>
        <p style="color:#536485; font-size:13px;">Ветка: <b>{{ cat_name }}</b> | Гео: <b>{{ country }}</b></p>
