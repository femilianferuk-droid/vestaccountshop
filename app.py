...
class User(db.Model):
    ...
    api_token = db.Column(db.String(100), unique=True, nullable=True) # Токен для Reseller API
...
# Инициализация структуры таблиц и автоматическая миграция
...
# --- Премиум Шаблон Tailwind Glassmorphism ---
BASE_TEMPLATE = """
<!DOCTYPE html>
...
<head>
    ...
    <title>VestAccs — Магазин аккаунтов</title>
    ...
    <div class="space-y-3">
        {% for p in my_purchases %}
            <div class="glass p-5 rounded-xl flex justify-between items-center flex-wrap gap-4">
                ...
                <div class="flex items-center gap-3">
                    <button class="px-4 py-2 text-xs font-bold text-white btn-shimmer rounded-xl" onclick="getCode({{ p.account_id }}, this)">Получить код</button>
...
def profile():
    ...
    # Авто-генерация токена API при первом посещении
    ...
    <div id="apiModal" class="fixed inset-0 bg-black/80 hidden z-50 flex items-center justify-center p-4">
        <div class="glass p-6 rounded-2xl w-full max-w-md border border-white/10">
            ...
            <button type="button" onclick="document.getElementById('apiModal').classList.add('hidden')" class="w-full py-3 text-xs font-bold uppercase text-white bg-white/5 border border-white/10 rounded-xl hover:bg-white/10 transition-all">
                Закрыть окно
            </button>
        </div>
    </div>
...
def documenting():
    html = """
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-black text-white tracking-tight shimmer-text">VESTACCS API</h2>
    ...
    <div class="glass p-6 rounded-2xl mb-6 border border-white/5">
        <h4 class="text-white font-bold text-sm uppercase tracking-wider mb-2 text-[#24A1DE]">Базовая Информация</h4>
        <p class="text-slate-400 text-xs leading-relaxed mb-3">Все запросы к API должны отправляться в формате JSON. Для авторизации передавайте ваш API-токен в Headers каждого запроса.</p>
        <div class="p-3 bg-slate-950 border border-white/5 rounded-xl font-mono text-xs text-slate-300">
            <span class="text-emerald-400">Base URL:</span> https://vestaccountshop.vercel.app/<br>
            <span class="text-purple-400">Header:</span> Authorization: Bearer {ВАШ_ТОКЕН}
        </div>
    </div>
    ...
    <div class="space-y-4">
        <div class="glass p-5 rounded-xl border border-white/5">
            ...
            <p class="text-slate-400 text-xs mb-3">Проверить баланс аккаунта реселлера.</p>
            <div class="bg-slate-950 p-3 rounded-xl font-mono text-[11px] text-slate-300">
                Response: {"status": "success", "username": "vestnik", "balance_rub": 5000}
            </div>
        </div>
        ...
        <div class="glass p-5 rounded-xl border border-white/5">
            ...
            <p class="text-slate-400 text-xs mb-3">Получить дерево цен для всех категорий и стран.</p>
            <div class="bg-slate-950 p-3 rounded-xl font-mono text-[11px] text-slate-300">
                Response: {"regular_Россия": 300, "aged_США": 450}
            </div>
        </div>
        ...
        <div class="glass p-5 rounded-xl border border-white/5">
            ...
            <p class="text-slate-400 text-xs mb-2">Автоматическая покупка аккаунта.</p>
            <p class="text-[10px] text-slate-500 mb-2">JSON Payload: {"category": "regular", "country": "США"}</p>
            <div class="bg-slate-950 p-3 rounded-xl font-mono text-[11px] text-slate-300">
                Response: {"status": "success", "purchase_id": 14, "phone": "+12025550143"}
            </div>
        </div>
    </div>
    """
...
def api_v1_buy():
    ...
    country_name = data.get("country") # Передавать точное русское название, напр. "США"
