...
def internal_server_error(e):
    """Если возникнет любая непредвиденная ошибка, она выведется на экран вместо пустой страницы 500"""
...
CRYPTO_BOT_URL = 'https://pay.cryptobots.net/api/' # Официальное рабочее веб-зеркало Crypto Pay

# Временный буфер для хранения промежуточных сессий добавления аккаунтов
...
# --- Изолированные Синхронные Обертки над Telethon (Решают проблему 500) ---
...
def tg_get_latest_code(session_str):
    async def _main():
        ...
        if not await client.is_user_authorized():
            await client.disconnect()
            return None, "Сессия устарела или закрыта владельцем"
        
        dialogs = await client.get_dialogs(limit=5)
        if not dialogs:
            await client.disconnect()
            return None, "На аккаунте нет активных чатов"
        
        # Перебираем последние 3 чата для надежности поиска кода
        ...
        await client.disconnect()
        return None, "5-значный код подтверждения в последних сообщениях не найден"
    return asyncio.run(_main())
...
<head>
    ...
    <style>
        body { font-family: 'Segoe UI', Arial, sans-serif; background-color: #121214; color: #e1e1e6; margin: 0; padding-bottom: 80px; }
...
<body>
    <div class="header">
        ...
        <div class="auth-buttons">
            ...
            {% else %}
                <span style="color: #a9a9b3;">Вы зашли как: <b>{{ session.get('username') }}</b></span>
    ...
    <div class="country-list">
        {% for country, count in counts.items() %}
            <div class="country-card">
                <h3>{{ country }}</h3>
                <p style="color: #a9a9b3;">В наличии: {{ count }} шт.</p>
                {% if session.get('user_id') %}
                    <a href="/shop" class="btn">Купить</a>
                {% else %}
                    <a href="/login" class="btn" onclick="alert('Для покупки необходимо авторизоваться!');">Купить</a>
                {% endif %}
            </div>
        {% endfor %}
    </div>
        ...
        {% for country, count in counts.items() %}
            <div class="country-card">
                ...
                <form action="/buy/{{ country }}" method="POST">
                    <button type="submit" class="btn" {% if count == 0 %}disabled style="background:#444; cursor:not-allowed;"{% endif %}>Купить</button>
                </form>
            </div>
        {% endfor %}
    </div>
...
def buy_account(country):
    ...
    if user.balance < price:
        flash('Недостаточно средств на балансе! Перейдите в профиль для пополнения.', 'error')
        return redirect(url_for('profile'))
    
    account = Account.query.filter_by(country=country, is_sold=False).first()
    if not account:
        flash(f'Аккаунты страны {country} в данный момент закончились.', 'error')
        return redirect(url_for('shop'))
    ...
    db.session.commit()
    flash(f'Успешная оплата! Номер: {account.phone}. Проверьте раздел "Мои покупки".', 'success')
...
def purchases():
    ...
    html = """
    <h2>Мои купленные аккаунты</h2>
    {% if not my_purchases %} <p>Вы еще не совершали покупок.</p> {% endif %}
    {% for p in my_purchases %}
        <div class="card" style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:10px;">
            <div><b>{{ p.country }}</b>: <code style="font-size:16px;">{{ p.phone }}</code></div>
            <div>
    ...
    function getCode(id, btn) {
        btn.innerText = 'Запрос к Telegram...';
        btn.disabled = true;
        fetch('/get_code/' + id).then(r => r.json()).then(data => {
            ...
            if (data.success) {
                output.innerText = 'Код: ' + data.code;
                output.style.color = '#6bff6b';
            } else {
                output.innerText = 'Ошибка: ' + data.message;
...
def get_code(account_id):
    if not session.get('user_id'): return jsonify({'success': False, 'message': 'Требуется вход'})
    purchase = Purchase.query.filter_by(user_id=session['user_id'], account_id=account_id).first()
    if not purchase: return jsonify({'success': False, 'message': 'Нет доступа к покупке'})
...
def profile():
    ...
    <div class="card">
        <h2>Мой профиль</h2>
        <p style="font-size:18px;">Баланс: <b style="color:#6bff6b;">{{ user.balance }} USDT</b></p>
        <hr style="border-color:#29292e; margin:20px 0;">
        <h3>Пополнение баланса через Crypto Bot</h3>
        <form action="/deposit" method="POST" style="max-width:300px;">
            ...
            <button type="submit" class="btn">Выставить счет</button>
        </form>
        {% if session.get('is_admin') %}
            <br><hr style="border-color:#29292e;"><br>
            <a href="/admin" class="btn" style="background:#ff9100; width:100%; box-sizing:border-box;">💼 Перейти в Админ Панель</a>
        {% endif %}
    </div>
    """
...
def deposit():
    ...
    payload = {'asset': 'USDT', 'amount': str(amount), 'description': f'Пополнение баланса пользователя ID: {session["user_id"]}'}
    ...
    try:
        ...
        if r.get('ok'):
            ...
            <div class="card" style="text-align:center;">
                <h2>Счет на оплату сформирован</h2>
                <p>Для зачисления <b>{{ amount }} USDT</b> перейдите по кнопке ниже в бота:</p>
                <p><a href="{{ url }}" target="_blank" class="btn" style="background:#00c853; padding:15px 30px; font-size:16px;">Оплатить в Telegram</a></p>
                <hr style="border-color:#29292e; margin:20px 0;">
                <form action="/check_invoice/{{ inv_id }}" method="POST"><button type="submit" class="btn btn-secondary">Я оплатил, проверить</button></form>
            </div>
            """
            return render_page(html, active_tab='profile', url=data['pay_url'], amount=amount, inv_id=inv.id)
        else:
            flash(f"Ошибка Crypto Bot: {r.get('explanation')}", 'error')
    except Exception as e:
        flash(f'Не удалось связаться с мерчантом: {e}', 'error')
    return redirect(url_for('profile'))
...
def check_invoice(invoice_id):
    ...
    try:
        ...
        if r.get('ok') and r['result']['items'] and r['result']['items'][0]['status'] == 'paid':
            ...
            inv.status = 'paid'
            db.session.commit()
            flash(f'Платеж подтвержден! На баланс зачислено {inv.amount} USDT.', 'success')
        else:
            flash('Оплата пока не зафиксирована системой Crypto Bot.', 'error')
    except Exception as e:
...
def admin_panel():
    if not session.get('is_admin'): return "Доступ запрещен", 403
    ...
    if request.method == 'POST' and 'change_balance' in request.form:
        ...
        if u:
            ...
            db.session.commit()
            flash(f'Баланс {u.username} изменен на {u.balance} USDT', 'success')
        else:
            flash('Такой пользователь не найден', 'error')
            
    html = """
    <h2>Панель Администратора (Vestnik)</h2>
    <div class="card">
        <h3>1. Добавление аккаунтов (Telethon MTProto)</h3>
        <form action="/admin/add_account" method="POST">
            <div class="form-group">
                <label>Номер телефона аккаунта (в международном формате)</label>
                <input type="text" name="phone" placeholder="+79991112233" required>
            </div>
            <button type="submit" class="btn">Отправить код авторизации</button>
        </form>
    </div>
    <div class="card">
        <h3>2. Изменение баланса пользователям</h3>
        <form method="POST"><input type="hidden" name="change_balance" value="1">
            <div class="form-group"><input type="text" name="username" placeholder="Логин покупателя" required></div>
            <div class="form-group"><input type="number" step="0.01" name="balance" placeholder="Новый баланс" required></div>
            <button type="submit" class="btn">Сохранить баланс</button>
        </form>
    </div>
    """
...
def admin_add_account():
    ...
    try:
        # Вызов изолированной сессии для отправки запроса кода
        session_str, phone_code_hash = tg_send_code(phone)
        
        # Сохраняем промежуточное состояние сессии в ОЗУ
        ...
        html = """
        <div class="card" style="max-width:400px; margin:0 auto;">
            <h3>Подтверждение входа Telegram</h3>
            <p>Система определила страну: <b>{{ country }}</b></p>
            <p>Код выслан на номер: <code>{{ phone }}</code></p>
            <form action="/admin/verify_account" method="POST">
                <input type="hidden" name="phone" value="{{ phone }}">
                <div class="form-group">
                    <label>Введите 5-значный код</label>
                    <input type="text" name="code" required autocomplete="off">
                </div>
                <button type="submit" class="btn" style="width:100%;">Завершить импорт</button>
            </form>
        </div>
        """
        return render_page(html, active_tab='profile', phone=phone, country=country)
    except Exception as e:
        flash(f'Ошибка Telethon при отправке запроса: {e}', 'error')
...
def admin_verify_account():
    ...
    if phone not in PENDING_REGISTRATIONS:
        flash('Регистрационная сессия не найдена или устарела.', 'error')
        return redirect(url_for('admin_panel'))
        
    data = PENDING_REGISTRATIONS[phone]
    try:
        # Авторизуем и получаем постоянную строку сессии
        final_session_string = tg_sign_in(phone, code, data['phone_code_hash'], data['session_str'])
        
        # Запись готового аккаунта в Postgres
        new_acc = Account(phone=phone, country=data['country'], session_string=final_session_string)
        db.session.add(new_acc)
        db.session.commit()
        
        del PENDING_REGISTRATIONS[phone]
        flash(f'Аккаунт {phone} ({data["country"]}) успешно добавлен в пул продаж!', 'success')
    except Exception as e:
        flash(f'Ошибка активации сессии (неверный код?): {e}', 'error')
        
    return redirect(url_for('admin_panel'))

if __name__ == '__main__':
    with app.app_context():
        db.create_all() # Автоматическое создание таблиц в СУБД Postgres при первом пуске
