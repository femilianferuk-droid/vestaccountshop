...
def purchases():
        ...
        {% for p in my_purchases %}
            <div class="glass p-5 rounded-xl flex justify-between items-center flex-wrap gap-4">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <button class="px-3 py-1.5 text-xs font-bold text-slate-300 bg-white/5 border border-white/10 rounded-xl hover:bg-white/10" onclick="getSession({{ p.account_id }}, this)">.session</button>
                </div>
    ...
    function getSession(id, btn) {
        const el = document.getElementById('out-'+id);
        el.classList.remove('hidden');
        el.innerText = 'Извлечение строки сессии...';
        el.style.color = '#94a3b8';
        fetch('/get_session_str/' + id).then(r => r.json()).then(data => {
            if(data.success) { el.innerText = 'STRING_SESSION: ' + data.session_string; el.style.color = '#60a5fa'; }
            else { el.innerText = 'Ошибка: ' + data.message; el.style.color = '#f87171'; }
        });
    }
    </script>
...
@app.route('/get_session_str/<int:account_id>')
def get_session_str(account_id):
    ...
    return jsonify({'success': True, 'session_string': account.session_string})
    ...
    <div id="apiModal" class="fixed inset-0 bg-black/80 hidden z-50 flex items-center justify-center p-4">
        <div class="glass p-6 rounded-2xl w-full max-w-md border border-white/10">
            ...
            <form action="/reset_api_token" method="POST" class="mb-2">
                <button type="submit" class="w-full py-2.5 text-xs font-bold uppercase text-white bg-rose-600/20 border border-rose-500/30 rounded-xl hover:bg-rose-600/40 transition-all">
...
# ИСПРАВЛЕНО: Добавлен бэкенд сброса токена реселлера
...
# --- Документация API ---
# ИСПРАВЛЕНО: Документация дополнена методами получения кодов и сессий по ID покупки
...
def documenting():
    ...
    <div class="glass p-6 rounded-2xl mb-6 border border-white/5">
        ...
        <div class="p-3 bg-slate-955 border border-white/5 rounded-xl font-mono text-xs text-slate-300">
            ...
            <span class="text-purple-400">Header:</span> Authorization: Bearer {ВАШ_API_ТОКЕН}
        </div>
    </div>
    ...
    <div class="space-y-4">
        <div class="glass p-5 rounded-xl border border-white/5">
            ...
            <p class="text-slate-400 text-xs mb-2">Проверка баланса личного кабинета.</p>
            <div class="bg-slate-950 p-3 rounded-xl font-mono text-[11px] text-slate-400">
                Response: {"balance_rub": 5000.0, "status": "success"}
            </div>
        </div>
        ...
        <div class="glass p-5 rounded-xl border border-white/5">
            ...
            <p class="text-slate-400 text-xs mb-2">Автоматический выкуп аккаунта из пула.</p>
            <p class="text-[10px] text-slate-500 mb-2">Payload: {"category": "regular", "country": "США"}</p>
            <div class="bg-slate-950 p-3 rounded-xl font-mono text-[11px] text-slate-400">
                Response: {"purchase_id": 482, "phone": "+12025550143", "status": "success"}
            </div>
        </div>
        ...
        <div class="glass p-5 rounded-xl border border-white/5">
            ...
            <p class="text-slate-400 text-xs mb-2">Запрос на извлечение 5-значного СМС кода авторизации аккаунта.</p>
            <div class="bg-slate-950 p-3 rounded-xl font-mono text-[11px] text-slate-400">
                Response: {"code": "84721", "status": "success"}
            </div>
        </div>
        ...
        <div class="glass p-5 rounded-xl border border-white/5">
            ...
            <p class="text-slate-400 text-xs mb-2">Экспорт StringSession (.session строки) для интеграции в сторонний софт.</p>
            <div class="bg-slate-950 p-3 rounded-xl font-mono text-[11px] text-slate-400">
                Response: {"session_string": "1BJWqV...", "status": "success"}
            </div>
        </div>
    </div>
    """
...
# ИСПРАВЛЕНО: Добавлен эндпоинт API получения кода по ID покупки
...
# ИСПРАВЛЕНО: Добавлен эндпоинт API выгрузки .session строки по ID покупки
...
def api_v1_get_session(purchase_id):
    ...
    if not pur: return jsonify({"status": "error", "message": "Purchase record not found"}), 404
    acc = Account.query.get(pur.account_id)
    return jsonify({"status": "success", "session_string": acc.session_string})
