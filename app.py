...
# ИСПРАВЛЕНО: Убран SQLite. По умолчанию сразу берется твой удаленный PostgreSQL, чтобы Vercel не падал из-за Read-Only системы
...
# ИСПРАВЛЕНО: Роут асинхронного получения кода без вложенных вызовов asyncio.run(), ломающих Vercel
...
def profile():
    ...
    <div class="glass p-6 rounded-2xl max-w-md mx-auto">
        ...
        <div class="flex flex-col gap-2">
            ...
            <button onclick="document.getElementById('promoModal').classList.remove('hidden')" class="w-full py-3 bg-white/5 border border-white/10 rounded-xl font-bold text-sm hover:bg-white/10 text-white transition-all">
                ПРОМОКОДЫ
            </button>
        </div>
    ...
    <div id="promoModal" class="fixed inset-0 bg-black/80 hidden z-50 flex items-center justify-center p-4">
        <div class="glass p-6 rounded-2xl w-full max-w-sm border border-white/10">
            ...
            <form action="/redeem_promo" method="POST" class="space-y-4">
                ...
                <button type="submit" class="w-full py-3 font-bold text-white btn-shimmer rounded-xl">Активировать промокод</button>
