...
app = Flask(__name__)
app.secret_key = 'vest_account_super_secret_key'
...
# ИСУПРАВЛЕНО: Используем официальное рабочее зеркало Crypto Pay API
CRYPTO_BOT_URL = 'https://pay.cryptobots.net/api/' 

# --- ИСПРАВЛЕНИЕ EVENT LOOP ДЛЯ TELETHON ---
# Создаем постоянный цикл событий в отдельном потоке, чтобы Telethon не терял сессии между HTTP-запросами
...
def run_telethon(coro):
    """Вспомогательная функция для безопасного запуска асинхронного кода Telethon"""
    future = asyncio.run_coroutine_threadsafe(coro, TELETHON_LOOP)
    return future.result()

# Временное хранилище активных клиентов в памяти
PENDING_CLIENTS = {}
...
<head>
    ...
    <style>
        ...
        .btn { background-color: #00b0ff; color: #fff; border: none; padding: 10px 20px; border-radius: 6px; cursor: pointer; font-weight: bold; text-decoration: none; display: inline-block; }
    ...
    <div class="header">
        ...
        <div class="auth-buttons">
            ...
            {% else %}
                ...
                <a href="/logout" class="btn-secondary" style="padding: 5px 10px;">Выйти</a>
            {% endif %}
        </div>
    </div>
...
def shop():
    ...
    return render_template_string(BASE_TEMPLATE + """
    {% block content %}
    <h2>Покупка аккаунтов (Стоимость: 50 у.е.)</h2>
    <div class="country-list">
        {% for country, count in counts.items() %}
            <div class="country-card">
                <h3>{{ country }}</h3>
...
def purchases():
    ...
    {% endblock %}
    """, my_purchases=my_purchases, active_tab='purchases')

# --- ИСПРАВЛЕНО: Безопасное извлечение кода через фоновый Event Loop ---
async def async_get_tg_code(session_string):
    client = TelegramClient(StringSession(session_string), API_ID, API_HASH)
    await client.connect()
    if not await client.is_user_authorized():
        await client.disconnect()
        return {"success": False, "message": "Сессия неавторизована"}
    
    dialogs = await client.get_dialogs(limit=1)
    if not dialogs:
        await client.disconnect()
        return {"success": False, "message": "Чаты пусты"}
    ...
    for m in messages:
        if m.text:
            ...
            if match:
                ...
                break
    await client.disconnect()
    return {"success": True, "code": code} if code else {"success": False, "message": "5-значный код не найден"}
...
def get_code(account_id):
    ...
    res = run_telethon(async_get_tg_code(account.session_string))
    return jsonify(res)
...
def profile():
    ...
    <div class="card">
        ...
        <form action="/deposit" method="POST" style="max-width:300px;">
            <div class="form-group"><label>Сумма пополнения</label><input type="number" name="amount" value="10" required></div>
            <button type="submit" class="btn">Пополнить через Crypto Bot</button>
        </form>
        {% if session.get('is_admin') %}<br><a href="/admin" class="btn" style="background:#ff9100;">Админ Панель</a>{% endif %}
    </div>
    {% endblock %}
    """, user=user, active_tab='profile')
...
def deposit():
    ...
    try:
        # Интеграция с зеркалом Crypto Pay API
        ...
        if r.get('ok'):
            ...
            <div class="card" style="text-align:center;">
                ...
                <form action="/check_invoice/{{ inv_id }}" method="POST"><button type="submit" class="btn btn-secondary">Проверить оплату</button></form>
            </div>
            {% endblock %}
            """, url=data['pay_url'], amount=amount, inv_id=inv.id, active_tab='profile')
...
def check_invoice(invoice_id):
    ...
    try:
        ...
        if r.get('ok') and r['result']['items'][0]['status'] == 'paid':
            ...
            flash('Оплата еще не поступила.', 'error')
    except Exception as e:
        flash(f'Ошибка: {e}', 'error')
    return redirect(url_for('profile'))

# --- ИСПРАВЛЕНО: Админ панель с удержанием Telethon-клиента в памяти ---
...
def admin_panel():
    ...
    <h2>Админ Панель</h2>
    <div class="card">
        <h3>Добавить аккаунт через Telethon</h3>
    ...
    <div class="card">
        ...
        <form method="POST"><input type="hidden" name="change_balance" value="1">
            ...
            <div class="form-group"><input type="number" step="0.1" name="balance" placeholder="Баланс" required></div>
            <button type="submit" class="btn">Сохранить</button>
        </form>
    </div>
    {% endblock %}
    """, users=User.query.all(), active_tab='profile')
...
def admin_add_account():
    ...
    try:
        # Выполняем асинхронный метод через защищенный фоновый Event Loop
        client, phone_code_hash = run_telethon(async_send_code(phone))
        
        # Сохраняем экземпляр запущенного клиента, привязанного к фоновому потоку
        ...
        <div class="card" style="max-width:400px; margin:0 auto;">
            <h3>Ввод кода Telegram</h3>
            <p>Страна: <b>{{ country }}</b> | Номер: <code>{{ phone }}</code></p>
            <form action="/admin/verify_account" method="POST">
                <input type="hidden" name="phone" value="{{ phone }}">
                <div class="form-group"><label>Введите код</label><input type="text" name="code" required></div>
        ...
        flash(f'Ошибка отправки кода: {e}', 'error')
        return redirect(url_for('admin_panel'))

async def async_sign_in(client, phone, code, phone_code_hash):
...
def admin_verify_account():
    ...
    try:
        # Завершаем авторизацию внутри того же потока, где создавался клиент
        session_string = run_telethon(async_sign_in(data['client'], phone, code, data['phone_code_hash']))
        
        new_acc = Account(phone=phone, country=data['country'], session_string=session_string)
...
if __name__ == '__main__':
    with app.app_context():
        db.create_all()
    app.run(host='0.0.0.0', port=5000, debug=False) # debug=False предотвращает двойной запуск фонового потока
