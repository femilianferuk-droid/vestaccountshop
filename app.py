...
with app.app_context():
    ...
    try:
        with db.engine.connect() as conn:
            ...
            conn.execute(text('ALTER TABLE "user" ADD COLUMN IF NOT EXISTS api_token VARCHAR(100);'))
            conn.commit()
    except Exception:
        pass
...
def buy_account(cat_slug, c_slug):
    ...
    if not account:
        flash('Данной гео-позиции нет в наличии.', 'error')
    ...
    flash(f'Успешно выдано! Номер: {account.phone}', 'success')
...
def purchases():
    ...
    function getCode(id, btn) {
        ...
        fetch('/get_code/' + id).then(r => r.json()).then(data => {
            if(data.success) { el.innerText = 'КОД АВТОРИЗАЦИИ: ' + data.code; el.style.color = '#34d399'; }
...
# ИСПРАВЛЕНО: Прямое скачивание .session файлом из веб-интерфейса покупателя
...
# --- ИСПРАВЛЕНО: Полноценная Документация API со всеми эндпоинтами цен, выкупа, кодов и файлов ---
...
# ИСПРАВЛЕНО: Эндпоинт API теперь отдает .session чистым бинарным файлом на скачивание (аттачмент)
